...
export function createCrawlerAction(iniDataIn, context, defaultCrawlerDb) {
  ...
  if (!iniData.crawler) iniData.crawler = {};

  const actionData = {};

  actionData.databasePath = mergeConfigItem(
    defaultCrawlerDb,
    null,
    iniData.crawler.databasePath
  );

  actionData.batchCount = mergeIntItem(
    constants.DEFCONF_CRAWLER_BATCHCOUNT,
    iniData.crawler.batchCount
  );
  ...
  for (let i = 0; i < actionData.folderBlacklist.length; i++) {
    actionData.folderBlacklist[i] = path.normalize(
      actionData.folderBlacklist[i]
    );
  }

  actionData.folderBlacklistSnippets = valiBlacklistSnippets(
    iniData.crawler.folderBlacklistSnippets
  );
  actionData.maxItemsPerContainer = mergeIntItem(
    constants.DEFCONF_MAX_ITEMS_PER_CONTAINER,
    iniData.crawler.maxItemsPerContainer
  );
  actionData.updateDirsAfterMinutes = mergeIntItem(
    constants.DEFCONF_CRAWLER_UPDATE_DIRS_AFTER_MINUTES,
    iniData.crawler.updateDirsAfterMinutes
  );

  actionData.weightingRating = mergeIntItem(
    constants.DEFCONF_CRAWLER_WEIGHTING_RATING,
    iniData.crawler.weightingRating
  );
  actionData.weightingRepeated = mergeIntItem(
    constants.DEFCONF_CRAWLER_WEIGHTING_REPEATED,
    iniData.crawler.weightingRepeated
  );
  actionData.weightingSeason = mergeIntItem(
    constants.DEFCONF_CRAWLER_WEIGHTING_SEASON,
    iniData.crawler.weightingSeason
  );
  actionData.weightingSelPow = mergeIntItem(
    constants.DEFCONF_CRAWLER_WEIGHTING_SELPOW,
    iniData.crawler.weightingSelPow
  );
...
export function createMainWindowAction(iniDataIn, context) {
  const iniData = iniDataIn;
  if (!iniData.mainWindow) iniData.mainWindow = {};
  ...
  if (context.tempCliFullscreen === true) actionData.fullscreen = true;
...
export function createSlideshowAction(iniDataIn, context) {
  const iniData = iniDataIn;
  if (!iniData.slideshow) iniData.slideshow = {};

  const actionData = {};

  actionData.autoPlay = mergeBoolItem(false, iniData.slideshow.autoPlay);

  actionData.detailsPosition = SlideshowReducer.valiDetailsPosition(
    valiString(iniData.slideshow.detailsPosition)
  );
  actionData.detailsShortenText = mergeIntItem(
    constants.DEFCONF_DETAILS_TEXT_SHORTEN,
    iniData.slideshow.detailsShortenText
  );
  actionData.detailsState = SlideshowReducer.getValidDetailsState(
    valiString(iniData.slideshow.detailsState),
    false
  );

  const tempCrawlerInfoPosition = valiString(
    iniData.slideshow.crawlerInfoPosition
  );
  actionData.crawlerInfoPosition = SlideshowReducer.valiCrawlerInfoPosition(
    tempCrawlerInfoPosition,
    actionData.detailsPosition
  );
  actionData.crawlerInfoShow = mergeBoolItem(
    false,
    iniData.slideshow.crawlerInfoShow
  );

  actionData.random = mergeBoolItem(
    constants.DEFCONF_RANDOM,
    iniData.slideshow.random
  );

  actionData.transitionTimeAutoPlay = mergeIntItem(
    constants.DEFCONF_TRANSITION_TIME_AUTOPLAY,
    iniData.slideshow.transitionTimeAutoPlay
  );
  actionData.transitionTimeManual = mergeIntItem(
    constants.DEFCONF_TRANSITION_TIME_MANUAL,
    iniData.slideshow.transitionTimeManual
  );
  actionData.timer = mergeIntItem(
    constants.DEFCONF_TIMER,
    iniData.slideshow.timer
  );
  ...
  if (context.tempCliAutoselect) {
    ...
    if (context.tempCliOpenContainer) {
      if (fs.existsSync(actionData.tempCliOpenContainer)) {
        ...
        log.info(
          `${_logKey}.createSlideshowAction - last file/dir doesn't exist (${
            context.tempCliOpenContainer
          })!`
        );
...
export function createSystemAction(iniDataIn, context, defaultLogFile) {
  const iniData = iniDataIn;
  if (!iniData.system) iniData.system = {};

  const actionData = {};

  actionData.exiftool = valiString(iniData.system.exiftool);

  actionData.powerSaveBlockTime = mergeIntItem(
    constants.DEFCONF_POWER_SAVE_BLOCK_TIME,
    iniData.system.powerSaveBlockTime
  );
  ...
  actionData.logLevelConsole = mergeConfigItem(
    constants.DEFCONF_LOGLEVEL_FILE,
    valiLogLevel(iniData.system.logLevelConsole)
  );

  actionData.mapUrlFormat = mergeConfigItem(
    constants.DEFCONF_META2MAPURL_FORMAT,
    valiUrl(iniData.system.mapUrlFormat)
  );

  actionData.logfile = defaultLogFile;
  if (
    iniData.system.logfile &&
    iniData.system.logfile.trim() !== constants.DEFCONF_LOG
  )
